...
LEGACY_FRONTEND_LABEL="com.ianwang.starmap.frontend"
LEGACY_FRONTEND_PLIST="$LAUNCH_AGENTS_DIR/$LEGACY_FRONTEND_LABEL.plist"
LEGACY_FRONTEND_RUNNER="$RUNTIME_BIN_DIR/start_frontend.sh"
UID_VALUE="$(id -u)"
...
launchctl bootout "gui/$UID_VALUE/$LEGACY_FRONTEND_LABEL" >/dev/null 2>&1 || true
launchctl disable "gui/$UID_VALUE/$LEGACY_FRONTEND_LABEL" >/dev/null 2>&1 || true
rm -f "$LEGACY_FRONTEND_PLIST" "$LEGACY_FRONTEND_RUNNER"
